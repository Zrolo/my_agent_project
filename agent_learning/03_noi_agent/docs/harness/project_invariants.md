...
3. `历史打卡` 是学生区内部独立入口；当前允许使用独立 in-app 路由：
   - `/app/history`
   - `/app/history/:checkin_id`

4. `历史打卡` 的角色仍然是**上下文选择器**，不是单纯展示列表：
   - 历史页负责紧凑浏览和选择记录
   - 历史详情页负责阅读该条记录的复盘、小测和知识兜底链路
   - 当前不再要求“选中记录后回到打卡工作台”这一种唯一交互
...
1. 复盘和知识卡都优先用“先破误解，再站正确桥”的写法：
   - 先说学生最容易误会的一句话
   - 再说真正要站稳的一句话

2. 知识卡里的 `algorithm_overview` 不是百科解释。
   - 只回答“这一步在整套方法里负责什么”
   - 不展开整套大讲义
...
1. `docs/common/version_governance.md`、`docs/common/changelog.md` 是项目级长期资产。

2. 后续只要发生成体系的版本更新，就必须同步维护版本治理与变更记录，不能只改其中一份。

3. 如果某次版本更新后，其中某一份文档无需改动，也必须在本轮确认其内容仍然有效，不能默认跳过。

4. 版本治理文档的职责长期固定为：
   - `version_governance.md`：写版本规则与版本规划
   - `changelog.md`：写已发生更新
