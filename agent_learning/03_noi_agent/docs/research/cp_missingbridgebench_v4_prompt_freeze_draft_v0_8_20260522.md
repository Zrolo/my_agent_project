# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.8
...
Change from v0.7: this draft adds an explicit forbidden-term list for title-only context after targeted run showed that operation-specific words such as "insert/query" could still appear. v0.8 is still a dev prompt, not a frozen prompt.
...
```json
{
  ...
  "title_only_forbidden_terms": [
    "比如", "例如", "插入", "查询", "合并", "距离", "点集", "节点", "字段", "cnt", "end", "dp[", "Trie", "线段树"
  ]
}
```
...
14. In `title_only` context, tutor-visible fields must not contain any item from `title_only_forbidden_terms` unless the same term is explicitly present in the public input packet.
...
19. In `title_only` context, do not use any item from `title_only_forbidden_terms`. For data-structure cases, use "一次操作" instead of insert/query/update-specific wording.
...
10. In `title_only` context, if the candidate response contains any item from `title_only_forbidden_terms`, set `needs_repair = "yes"` and include that phrase in `repair_target_sentences`.
...
13. In `title_only` context, remove every item from `title_only_forbidden_terms`; replace operation-specific wording with "一次操作" or "一步操作".
