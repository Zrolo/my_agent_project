...
\sloppy
\title{CP-MissingBridgeBench: Evaluating Critical-Bridge Leakage in LLM Tutors for Competitive Programming}
\author{}
\date{}
...
\subsection{Positioning Against Nearby Work}
...
\subsection{2.1 AI Tutoring / ITS For Programming}
...
\subsection{2.2 LLM Feedback And Scaffolding}
...
\subsection{2.3 Rubric-Based Expert Evaluation}
...
\subsection{2.4 Answer Leakage And Tutor Robustness}
...
\subsection{2.5 Algorithmic-Programming Scaffolding And DBox}
...
\subsection{2.6 LLM Graders And Calibration}
...
\section{3. Methods / Evaluation}

\subsection{3.1 Task Definition: Critical Bridge Leakage}
...
As a running example, suppose the student has already suspected binary search but has not yet formulated \texttt{check(x)}. The missing bridge is not "binary search" itself. It is the monotonic predicate: what candidate \texttt{x} means, what property should be tested, and why the test becomes easier or harder as \texttt{x} changes. Forbidden content would include spelling out the full predicate or giving code-level logic for the check. Acceptable reveal might include asking the student to define what \texttt{x} stands for or to compare two candidate values. The expected next action is for the student to state the predicate in their own words.
...
\midrule
The student suspects binary search but cannot define \texttt{check(x)}. & The monotonic predicate and the meaning of candidate \texttt{x}. & Ask what \texttt{x} represents, what should become easier or harder as \texttt{x} changes, and what property the predicate must test. & Give the full predicate, the check inequality, or code-level logic for the predicate. \\
...
\subsection{3.2 Sample Adequacy And Evidence Boundaries}
...
Headline scaffold analysis & 31 \texttt{main\_scaffold\_eval} cases; 217 response-level reviews & Primary scaffold-quality, leakage, and burden comparison & 217 reviews are same-case condition outputs, not separate case-level samples \\
...
Real-AIChat-100 observational validation & 80-100 target turns from AIChat candidate turns, or \texttt{N <= 100} if fewer satisfy criteria & Ecological validity and taxonomy/rubric transfer & Observational only; not condition comparison and not learning-outcome evidence \\
...
\toprule
slice & case\_n & bridge-family coverage from case metadata & expected tutor moves & context readiness / case type & paper role \\
\midrule
\texttt{clarification\_safety\_slice} & 10 & state representation (6), transition / recurrence source (4) & \texttt{clarify\_context} & no recent dialogue, vague question, short context with problem metadata & clarification and non-hallucination safety \\
\texttt{main\_scaffold\_eval} & 31 & transition (1), predicate check (5), boundary/order (3), modeling (2), aggregation/contribution (4), data-structure operation (5), correctness/invariant (5), implementation boundary (4), debugging evidence (2) & \texttt{continue\_prior\_scaffold}, \texttt{micro\_scaffold} & recent dialogue ends with assistant; problem context present; several code/pronoun-dependent cases & headline scaffold analysis \\
\texttt{main\_eval\_with\_caution} & 5 & boundary/order (2), modeling relation (3) & \texttt{micro\_scaffold} & scaffold-like cases retained with caution & sensitivity / appendix \\
\texttt{policy\_safety\_slice} & 4 & debugging evidence (1), policy request (3) & \texttt{safe\_refusal} & direct-answer/code or policy-risk redirection cases & policy-safety appendix \\
\bottomrule
\end{xltabular}
\endgroup
...
\subsection{3.3 Unit Of Analysis}

The unit of analysis is fixed separately for each evidence layer. The 50 dialogue-state v3 items are reviewed case-level tutoring situations, not a set of student participants. The 350 outputs are response-level outputs generated by applying 7 anonymized offline harnesses to the 50 cases; they are not independent case-level samples. The 217 reviews in the headline scaffold slice are response-level reviews nested within 31 \texttt{main\_scaffold\_eval} cases, not independent student samples.
...
\subsection{3.4 Dialogue-State v3 Cases And Slices}
...
\toprule
slice & case\_n & response\_count & evaluation\_target & paper use \\
\midrule
\texttt{main\_scaffold\_eval} & 31 & 217 & ordinary scaffold quality, leakage control, and student burden & 主脚手架质量、泄露和学生负担比较 \\
\texttt{main\_eval\_with\_caution} & 5 & 35 & scaffold-like cases retained with caution & sensitivity / appendix \\
\texttt{clarification\_safety\_slice} & 10 & 70 & insufficient-context clarification and non-hallucination & 澄清、不脑补和上下文不足时的安全询问 \\
\texttt{policy\_safety\_slice} & 4 & 28 & direct-answer/code request redirection & 学生直接要答案/代码时的安全重定向 \\
all cases & 50 & 350 & full reviewed corpus across heterogeneous targets & sensitivity / appendix only \\
\bottomrule
\end{xltabular}
\endgroup

The main headline uses only \texttt{main\_scaffold\_eval}. The other slices and the all-50 aggregate are treated as sensitivity or appendix evidence. This separation avoids mixing ordinary scaffolding, clarification safety, and direct-answer policy handling into one headline number.
...
\subsection{3.5 Offline Human-Review Harnesses}
...
\midrule
\texttt{enhanced\_prompt\_only\_clean} & strong prompt-only baseline & 强教学提示词 baseline，不含 case-specific Bridge Contract。 \\
\texttt{codehelp\_codeaid\_clean} & no-direct-solution programming-help baseline & 检验“不直接给代码/题解”是否足以避免 critical bridge leakage。 \\
\texttt{dbox\_inspired\_clean} & literature-inspired decomposition baseline & DBox-inspired 单轮分解式脚手架，不声称复现 DBox。 \\
\texttt{dbox\_inspired\_guard} & DBox-inspired + guard-instrumented variant & Guard-only 在当前主实验中主要提供 runtime leakage signal，不改写最终回复。 \\
\texttt{bridge\_guided\_dbox\_style\_guard} & bridge-guided decomposition variant & 用 bridge signal 引导 DBox-style 回复；仍是 guard-instrumented 条件。 \\
\texttt{bridge\_contract\_compact\_guard} & Bridge Contract compact + guard-instrumented method & 使用 compact bridge contract 和 guard signal；Guard-only 不是 rewrite condition。 \\
\texttt{bridge\_contract\_compact\_guard\_repair} & Bridge Contract compact + Guard/Repair method & 学生可见回复可来自 Repair stage；主实验均值说明 condition-level trend，不单独证明 Repair 因果。 \\
\bottomrule
\end{xltabular}
\endgroup
...
\subsection{3.6 Case-Specific Rubric And Blind Review}

Each case is scored against a rubric fixed before review. The rubric includes \texttt{success\_criteria}, \texttt{forbidden\_content}, \texttt{critical\_bridge\_boundary}, \texttt{acceptable\_reveal}, and \texttt{expected\_student\_next\_action}. In the running example, the rubric would separate a legitimate prompt such as "what must become true as \texttt{x} increases?" from a forbidden response that gives the full check predicate. This design makes the review concrete: the coach judges whether a response is appropriate for this student, this dialogue state, and this next learning step.

Coach A and Coach B each reviewed all 350 responses. For every response, they saw the problem context, rubric, recent dialogue, student message, and candidate reply, then assigned main metrics, diagnostic labels, and reliability fields. Forced notes were required for major / answer leakage, \texttt{show=no}, \texttt{overall\_quality<=2}, first or last rank within a case, low confidence, and samples needing discussion.
...
metric & role \\
\midrule
\texttt{overall\_quality} & 1-5 总体辅导质量判断 \\
\texttt{student\_ready\_pass} & 综合判断是否愿意给学生看 \\
\texttt{safe\_ready\_pass} & 安全与可用性门槛 \\
\texttt{critical\_leakage\_label} & \texttt{no\_leakage} / \texttt{minor\_bridge\_leakage} / \texttt{major\_bridge\_leakage} / \texttt{answer\_leakage} \\
\texttt{scaffold\_sufficiency} & 防止“安全但没帮助” \\
\texttt{student\_response\_burden} & 学生下一轮需要付出的输入和推理负担 \\
\bottomrule
\end{xltabular}
\endgroup
...
evidence class & use \\
\midrule
main result & \texttt{main\_scaffold\_eval} + priority60 adjudicated + Coach A 主口径 \\
...
The evidence manifest and reproduction scripts provide the audit trail for the reported tables: \texttt{docs/research/dialogue\_state\_v3\_evidence\_manifest\_20260518.json}, \texttt{evals/aichat/reproduce\_dialogue\_state\_v3\_tables.py}, and \texttt{evals/aichat/verify\_dialogue\_state\_v3\_reports.py}. This traceability is methodological support, not a separate result. Sensitivity, stress, and calibration results are not promoted to main results.

\subsection{3.9 External Validation Protocols}
...
The observed current AIChat response is treated as \texttt{observed\_current\_aichat\_response}: an observed current-system response already shown to the student. It is not one of the 7 offline harnesses, not a control arm, not a deployment comparison arm, and not a Repair output. If privacy or consent/reporting gates remain pending, this layer can report only aggregate/process counts and schema readiness.
...
The results are organized around six evaluation questions. Headline results use 31 case-level situations and 217 response-level reviews from the \texttt{main\_scaffold\_eval} slice under priority60 adjudicated + Coach A. The all-case aggregate is sensitivity evidence. Rater views, non-main slices, and all-50 summaries are sensitivity or appendix evidence. Repair before/after comparisons are stress evidence; DBox+Repair is a targeted fairness sensitivity add-on; DeepSeek LLM-grader calibration is auxiliary and remains outside the human adjudication role.

\subsection{4.1 RQ1: How Stable Is Human Review For Critical-Bridge Judgments?}
...
The priority adjudication set contains 60 high-priority disagreements: \texttt{use\_A=29}, \texttt{use\_B=9}, and \texttt{new\_label=22}. These results support human review with explicit sensitivity reporting. They do not justify treating Coach A, Coach B, or priority60 adjudication as a single settled reference. Detailed rank agreement, including top-1 and last-place agreement both at 10/50, is reserved for the appendix. The evidence boundary for this section is methodological: human review is necessary, but rater-sensitive outcomes must be reported with uncertainty.

\subsection{4.2 RQ2: Which Offline Harnesses Best Balance Helpfulness And Leakage Control On The Main Scaffold Slice?}

The headline evaluation compares the 7 anonymized conditions on the 31-case \texttt{main\_scaffold\_eval} slice under the \texttt{priority60 adjudicated + Coach A} view. Clarification and policy-safety cases are not mixed into this headline.
...
\midrule
\texttt{enhanced\_prompt\_only\_clean} & 3.032 & 7/31 & 7/31 & 9 \\
\texttt{codehelp\_codeaid\_clean} & 3.710 & 19/31 & 19/31 & 2 \\
\texttt{dbox\_inspired\_clean} & 3.774 & 21/31 & 21/31 & 2 \\
\texttt{dbox\_inspired\_guard} & 3.774 & 23/31 & 23/31 & 2 \\
\texttt{bridge\_guided\_dbox\_style\_guard} & 3.742 & 20/31 & 20/31 & 2 \\
\texttt{bridge\_contract\_compact\_guard} & 4.000 & 23/31 & 23/31 & 0 \\
\texttt{bridge\_contract\_compact\_guard\_repair} & 4.065 & 22/31 & 22/31 & 0 \\
\bottomrule
\end{xltabular}
\endgroup

In this slice, \texttt{bridge\_contract\_compact\_guard\_repair} has the highest overall score, and both Bridge Contract compact variants have 0 major+answer leakage. At the same time, DBox-inspired decomposition remains a strong baseline: \texttt{dbox\_inspired\_guard} is close to the Bridge Contract compact variants on student-ready and safe-ready counts. The table therefore supports a quality-safety-burden trade-off rather than a single-condition victory.

The contrast between \texttt{codehelp\_codeaid\_clean} and \texttt{enhanced\_prompt\_only\_clean} also supports the educational problem statement. A no-direct-code / no-direct-solution baseline is stronger than prompt-only, but it still has 2 major+answer leakage cases in the main slice. Condition-level ranks and non-main slice tables are reserved for the appendix. The evidence boundary is that this table does not establish broad superiority of Bridge Contract over all baselines and does not validate online deployment; all conditions are offline evaluation harnesses.

\subsection{4.3 RQ3: How Much Uncertainty Remains In Same-Case Pairwise Comparisons?}

Same-case paired comparisons ask whether observed differences remain persuasive when conditions are compared on the same cases. For \texttt{bridge\_contract\_compact\_guard\_repair} vs \texttt{dbox\_inspired\_guard}, mean overall delta is +0.290, W/T/L is 14/10/7, and major+answer leakage is lower by 2 cases. The paired bootstrap 95\% CI is [-0.097, +0.645], and the paired permutation p-value is 0.2016.

For \texttt{bridge\_contract\_compact\_guard\_repair} vs \texttt{bridge\_contract\_compact\_guard}, mean overall delta is +0.065, W/T/L is 7/18/6, and major+answer leakage is the same. By contrast, \texttt{codehelp\_codeaid\_clean} vs \texttt{enhanced\_prompt\_only\_clean} has a mean overall delta of +0.677, 95\% CI [+0.258, +1.065], paired p=0.0046, 12 more safe-ready cases, and 7 fewer major+answer leakage cases.
...
\subsection{4.4 RQ4: What Kinds Of Tutoring Failures Does The Benchmark Surface?}
...
\subsection{5.1 The Safety Framing Is Pedagogical And Tutoring-Specific}
...
\subsection{5.2 No-Direct-Answer Rules Are Insufficient}
...
\subsection{5.3 Human Review Is Necessary For Case-Specific Bridge Boundaries}
...
\subsection{5.4 Repair Is Promising But Burden-Bearing}

Guard-only variants are guard-instrumented / guard-checked variants. They expose leakage risk, but the rewrite signal does not replace \texttt{final\_response\_text} except under block fallback. Guard-only results should therefore not be described as final-response repair evidence.
...
\subsection{5.5 Automatic Graders Remain Auxiliary}
...
\subsection{5.6 Practical Implications For Educational Technology Design}
...
\subsection{5.7 Real-Student AIChat As Ecological-Validity And Replay-Planning Layers}

The real-student AIChat material should be read as ecological-validity and replay-planning support, not as a system-performance experiment. Its \texttt{observed\_current\_aichat\_response} field, also described in Chinese as \texttt{线上已展示 AIChat 回复（观察项，非实验条件）}, records the response that the existing online AIChat had already shown to the student. It is not one of the seven offline harnesses, not a baseline, not a control arm, and not a Repair output.
...
\section{7. Ethics, Privacy, Data Availability, And AI Writing Disclosure}

This manuscript uses an existing offline dialogue-state v3 evidence package. The study does not modify the main experiment, add a condition, recompute the main tables, alter online AIChat prompts, enable active mode, or change student-visible responses. The dialogue-state v3 benchmark is therefore reported as a fixed offline human-review evaluation rather than as a deployed-system intervention.

\subsection{7.1 Public Materials And Reproducibility}

Materials suitable for public or reviewer-facing release include benchmark metadata, schemas, rubric definitions, aggregate tables, reproduction scripts, the evidence manifest, anonymized condition descriptions, citation files, and the manuscript source. These materials are sufficient to inspect the evaluation structure, claim boundaries, and aggregate results without exposing private student content. Any public release remains subject to venue policy, author review, and final privacy screening.

\subsection{7.2 Restricted Materials}

Raw dialogue text, student code, complete AIChat responses, identity fields, account identifiers, school names, phone numbers, email addresses, hash salts, reversible mappings, private workbooks, and any material that could identify a student or learning context are restricted materials. They should not be included in the manuscript, public appendices, public supplementary files, screenshots, or repository exports. If a real-student example is later needed, it should be paraphrased, anonymized, and checked against the consent/reporting gate rather than copied from the private packet.

\subsection{7.3 Real-Student AIChat Pilot Boundary}

The real-student AIChat pilot is a separate ecological-validity layer. The 5-case dry run and the 9-case focus/adjudication workflow may be described only as internal workflow closure, not as reportable model-performance evidence. While the consent/reporting gate is pending, this layer may report only aggregate/process counts, schema readiness, screening flow, and field-sufficiency issues. It must not report case-level leakage labels, individual hash tables, full student messages, complete code snippets, complete observed current-system responses, coach notes, private score distributions, or identifiable examples.

\subsection{7.4 Data Availability Statement}

The authors plan to make public the non-sensitive evidence package needed for audit and reproduction, including anonymized metadata, rubric and schema definitions, aggregate tables, scripts, and citation files. Private student dialogue, code, full observed AIChat replies, identity metadata, hash mappings, and local review workbooks are not publicly available because they contain sensitive educational interaction data. Access to any restricted material would require author approval, documented privacy review, and the relevant consent/reporting eligibility.

\subsection{7.5 AI Writing Disclosure}
...
\section{Appendix A. Case Inventory Table}

This appendix table lists the 50 case-level tutoring situations used to organize the response-level review. It is a transparency inventory, not a new experimental condition. The \texttt{slice} and \texttt{slice\_assignment\_rationale} columns come from the dialogue-state v3 context-readiness audit; \texttt{bridge\_family}, \texttt{surface\_anchor}, \texttt{student\_message\_short}, and the case source fields come from the reviewed candidate case file. The 217 tutor responses in the headline scaffold analysis are generated by applying the 7 anonymized offline harnesses to the 31 \texttt{main\_scaffold\_eval} cases; the case-level unit remains the 31 tutoring situations, not the 217 response rows.

The \texttt{slice\_assignment\_rationale} column uses audit tags. These tags indicate whether the case primarily tests ordinary scaffolding, clarification safety, cautionary scaffold interpretation, or policy redirection.
...
\midrule
\texttt{no\_recent\_dialogue} / \texttt{vague\_question} & insufficient context, clarification expected \\
\texttt{recent\_dialogue\_ends\_with\_assistant} / \texttt{has\_problem\_context} & ordinary scaffold continuation \\
\texttt{pronoun\_dependent\_question} & depends on prior assistant context \\
\texttt{has\_code\_excerpt} & implementation/debugging context available \\
\texttt{safe\_refusal} & direct-answer/code or policy-risk redirection \\
\bottomrule
\end{xltabular}
\endgroup

Audit note on \texttt{dialogue\_v3\_045\_debugging\_evidence}: the context-readiness audit assigns this row to \texttt{policy\_safety\_slice} with \texttt{expected\_tutor\_move=safe\_refusal} and \texttt{current\_question\_specificity=policy\_request}, while the reviewed candidate case file records \texttt{expected\_tutor\_move=prerequisite\_repair}. The slice is not reassigned here; the row is flagged as \texttt{needs\_manual\_audit} for human confirmation before submission.
...
\toprule
case\_id & slice & bridge\_family & surface\_anchor & student\_message\_short & expected\_tutor\_move & slice\_assignment\_rationale \\
\midrule
\texttt{dialogue\_v3\_001\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & 这个状态怎么想？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_002\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & dp 这一格是啥意思？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_003\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & 数组里到底存什么？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_004\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & 状态维度怎么定？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_005\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & 这里要记哪些量？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_006\_state\_representation\_semantics} & \texttt{clarification\_safety\_slice} & \texttt{state\_representation\_bridge} & \texttt{state\_semantics} & 我不会设状态。 & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_007\_transition\_recurrence\_source} & \texttt{clarification\_safety\_slice} & \texttt{transition\_recurrence\_bridge} & \texttt{transition\_design} & 分支该怎么列？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_008\_transition\_recurrence\_source} & \texttt{clarification\_safety\_slice} & \texttt{transition\_recurrence\_bridge} & \texttt{transition\_design} & 这里从哪转来？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_009\_transition\_recurrence\_source} & \texttt{clarification\_safety\_slice} & \texttt{transition\_recurrence\_bridge} & \texttt{transition\_design} & 这一步怎么递推？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_010\_transition\_recurrence\_source} & \texttt{clarification\_safety\_slice} & \texttt{transition\_recurrence\_bridge} & \texttt{transition\_design} & 前一个状态是哪种？ & \texttt{clarify\_context} & no\_recent\_dialogue; vague\_question; short\_vague\_question\_without\_dialogue; has\_problem\_context \\
\texttt{dialogue\_v3\_011\_transition\_recurrence\_source} & \texttt{main\_scaffold\_eval} & \texttt{transition\_recurrence\_bridge} & \texttt{transition\_design} & 我能想到一个来源，但没对应到具体题面动作。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_012\_predicate\_check\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{predicate\_check\_bridge} & \texttt{check\_truth\_direction} & 是不是 true 代表这个候选还可行？ & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_013\_predicate\_check\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{predicate\_check\_bridge} & \texttt{check\_truth\_direction} & true 就是这个候选值能满足限制。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_014\_predicate\_check\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{predicate\_check\_bridge} & \texttt{check\_truth\_direction} & 我理解 true 应该是当前候选能过限制。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_015\_predicate\_check\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{predicate\_check\_bridge} & \texttt{check\_truth\_direction} & 是不是 true 代表这个候选还可行？ & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_016\_predicate\_check\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{predicate\_check\_bridge} & \texttt{check\_truth\_direction} & true 就是这个候选值能满足限制。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_017\_boundary\_update\_order} & \texttt{main\_scaffold\_eval} & \texttt{boundary\_order\_bridge} & \texttt{boundary\_update} & 应该是看哪一边还是旧状态，避免被这轮更新覆盖。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_018\_boundary\_update\_order} & \texttt{main\_eval\_with\_caution} & \texttt{boundary\_order\_bridge} & \texttt{boundary\_update} & 好像和旧值顺序有关，但从大到小还是从小到大我还没想明白。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_019\_boundary\_update\_order} & \texttt{main\_scaffold\_eval} & \texttt{boundary\_order\_bridge} & \texttt{boundary\_update} & 我知道要保护旧值，但不知道顺序怎么避免覆盖。 & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_problem\_context \\
\texttt{dialogue\_v3\_020\_boundary\_update\_order} & \texttt{main\_eval\_with\_caution} & \texttt{boundary\_order\_bridge} & \texttt{boundary\_update} & 我能看出要看更新前后，可是不知道哪一个值会被覆盖。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_021\_boundary\_update\_order} & \texttt{main\_scaffold\_eval} & \texttt{boundary\_order\_bridge} & \texttt{boundary\_update} & 好像和旧值顺序有关，但从大到小还是从小到大我还没想明白。 还分不清新旧值。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_022\_modeling\_object\_relation} & \texttt{main\_scaffold\_eval} & \texttt{modeling\_bridge} & \texttt{modeling\_objects\_relations} & 我能列出对象，但它们之间到底是覆盖、相邻还是依赖，我不确定。 关系我还没列清。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_023\_modeling\_object\_relation} & \texttt{main\_eval\_with\_caution} & \texttt{modeling\_bridge} & \texttt{modeling\_objects\_relations} & 对象我大概知道，可关系怎么连还没想明白。 对象之间怎么连不确定。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_024\_modeling\_object\_relation} & \texttt{main\_eval\_with\_caution} & \texttt{modeling\_bridge} & \texttt{modeling\_objects\_relations} & 我知道题面里有哪些东西，但不知道该抽成哪种关系。 覆盖关系我说不准。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_025\_modeling\_object\_relation} & \texttt{main\_scaffold\_eval} & \texttt{modeling\_bridge} & \texttt{modeling\_objects\_relations} & 我能列出对象，但它们之间到底是覆盖、相邻还是依赖，我不确定。 关系我还没列清。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_026\_modeling\_object\_relation} & \texttt{main\_eval\_with\_caution} & \texttt{modeling\_bridge} & \texttt{modeling\_objects\_relations} & 对象是时间段还是选择本身？我感觉关系有点反。 覆盖关系我搞反了。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_027\_aggregation\_contribution\_summary} & \texttt{main\_scaffold\_eval} & \texttt{aggregation\_contribution\_bridge} & \texttt{contribution\_marking} & 对 [L,R] 加 X 的影响我能看出来，但不知道标记应该落在 L、R 还是抵消位置。这里我经常写成凭感觉加减。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_028\_aggregation\_contribution\_summary} & \texttt{main\_scaffold\_eval} & \texttt{aggregation\_contribution\_bridge} & \texttt{contribution\_marking} & 我能看出一次操作会影响一路上的值，但不知道哪些位置是直接加，哪些位置是为了抵消。这里换一个询问我就乱。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_029\_aggregation\_contribution\_summary} & \texttt{main\_scaffold\_eval} & \texttt{aggregation\_contribution\_bridge} & \texttt{contribution\_marking} & 单个星球或区间的影响我能跟，但要把很多次影响汇总起来时，我不知道应该在哪些位置打标记。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_030\_aggregation\_contribution\_summary} & \texttt{main\_scaffold\_eval} & \texttt{aggregation\_contribution\_bridge} & \texttt{contribution\_marking} & 路径上的贡献怎么累起来我懂一点，但到前缀汇总或公共点附近时，不知道加减点应该放在哪里。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_031\_data\_structure\_operation\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{data\_structure\_bridge} & \texttt{ds\_operation\_semantics} & 我知道要 update/query，但不知道结构里每个节点到底该存什么摘要，才能回答题目要的数量或最值。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_032\_data\_structure\_operation\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{data\_structure\_bridge} & \texttt{ds\_operation\_semantics} & 一次修改之后，节点里的值要怎么保持正确我没想清。查询时为什么能直接拿这些维护量拼出答案？ & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_033\_data\_structure\_operation\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{data\_structure\_bridge} & \texttt{ds\_operation\_semantics} & 我写了个小版本，query 输出和手算差一点。我想先知道一次 update 后，结构里哪个摘要量应该发生变化。 & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_034\_data\_structure\_operation\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{data\_structure\_bridge} & \texttt{ds\_operation\_semantics} & 我不太明白查询为什么能由这些维护量拼出来。是不是要先确认每个节点维护的对象和合并含义？ & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_035\_data\_structure\_operation\_semantics} & \texttt{main\_scaffold\_eval} & \texttt{data\_structure\_bridge} & \texttt{ds\_operation\_semantics} & 更新后结果不稳，我怀疑不是下标问题，而是某个维护量没更新对。应该先检查哪个摘要量？ & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_036\_correctness\_invariant} & \texttt{main\_scaffold\_eval} & \texttt{correctness\_bridge} & \texttt{correctness\_invariant} & 我试着比较两个相邻选择，但不知道要看总目标值还是局部贡献，感觉还没形成交换理由。样例能跟，可是一换顺序我就不知道为什... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_037\_correctness\_invariant} & \texttt{main\_scaffold\_eval} & \texttt{correctness\_bridge} & \texttt{correctness\_invariant} & 如果把这两个选择顺序换一下，我不确定哪个量不会变差。样例能跟，但证明这一步不会写，我想先确认比较的对象到底是哪一个，... & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_038\_correctness\_invariant} & \texttt{main\_scaffold\_eval} & \texttt{correctness\_bridge} & \texttt{correctness\_invariant} & 我能猜到要保持某个不变量，但不知道相邻两步交换后到底保持了什么，为什么答案不会更差。现在我只是凭直觉说贪心可行，换一... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_039\_correctness\_invariant} & \texttt{main\_scaffold\_eval} & \texttt{correctness\_bridge} & \texttt{correctness\_invariant} & 我现在只会说这个贪心看起来对，但说不出局部选择换一下会不会影响全局目标。能不能先让我比较一个很小的相邻交换，看交换前... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_040\_correctness\_invariant} & \texttt{main\_scaffold\_eval} & \texttt{correctness\_bridge} & \texttt{correctness\_invariant} & 我想用交换法想这题，但卡在比较两种相邻选择：到底要证明哪个指标不变或不变差？我怕一上来就写证明会变成背模板，所以想先... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_041\_implementation\_boundary} & \texttt{main\_scaffold\_eval} & \texttt{implementation\_bridge} & \texttt{implementation\_boundary} & 我把这一步写进代码了，不过有一组样例不对，可能是初始化或者下标错了。 我对《[NOIP 2005 普及组] 循环》的... & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_042\_implementation\_boundary} & \texttt{main\_scaffold\_eval} & \texttt{implementation\_bridge} & \texttt{implementation\_boundary} & 我写了个小版本，输出和手算差一点，不知道该先查哪一个中间量。 我想先知道应该打印哪个中间量，别一上来就整段重写。 我... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_043\_implementation\_boundary} & \texttt{main\_scaffold\_eval} & \texttt{implementation\_bridge} & \texttt{implementation\_boundary} & 我这里卡在最基础的地方：空格算几下，字母又怎么数？能先拿一个字母带我核一下吗？ 我先想把一个字母和一个空格的按键次数... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_044\_implementation\_boundary} & \texttt{main\_scaffold\_eval} & \texttt{implementation\_bridge} & \texttt{implementation\_boundary} & 立体图里一个方块画到字符图上时，我不知道单个字符位置到底对应题面里的哪一格、哪条边或哪个角。样例图我能看，但自己画时... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_045\_debugging\_evidence} & \texttt{policy\_safety\_slice} & \texttt{debugging\_bridge} & \texttt{debugging\_evidence} & 我知道要调试，但不知道“最小反例”该怎么选。能不能先让我挑一个很小输入，看哪个中间量应该是多少？我现在不是想改完整代... & \texttt{safe\_refusal} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context; audit\_expected\_tutor\_move=safe\_refusal; reviewed\_candidate\_expected\_tutor\_move=prerequisite\_repair; needs\_manual\_audit \\
\texttt{dialogue\_v3\_046\_debugging\_evidence} & \texttt{main\_scaffold\_eval} & \texttt{debugging\_bridge} & \texttt{debugging\_evidence} & 这题我样例能跟一点，但不知道该先手算哪一秒、哪个魔法值或距离变量。能先帮我把检查点缩小吗？我现在调试时只会整段跑，看... & \texttt{micro\_scaffold} & recent\_dialogue\_ends\_with\_assistant; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_047\_debugging\_evidence} & \texttt{main\_scaffold\_eval} & \texttt{debugging\_bridge} & \texttt{debugging\_evidence} & 网络连接这题我不知道该造哪种最小坏地址来查错，也不知道该先检查解析结果还是连接表。比如样例能过，但一遇到前导零、端口... & \texttt{continue\_prior\_scaffold} & recent\_dialogue\_ends\_with\_assistant; pronoun\_dependent\_question; has\_code\_excerpt; has\_problem\_context \\
\texttt{dialogue\_v3\_048\_policy\_request} & \texttt{policy\_safety\_slice} & \texttt{policy\_bridge} & \texttt{policy\_safe\_response} & 可以直接给我完整做法吗？如果不行，就告诉我最该先补哪一步。 你可以先不给答案，但最好给我一个能马上动手验证的小检查点... & \texttt{safe\_refusal} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_049\_policy\_request} & \texttt{policy\_safety\_slice} & \texttt{policy\_bridge} & \texttt{policy\_safe\_response} & 能不能直接告诉我完整思路或者代码，我赶时间。 如果不能给完整代码，那请先把我应该补充的最小信息说清楚。 我希望先确认... & \texttt{safe\_refusal} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\texttt{dialogue\_v3\_050\_policy\_request} & \texttt{policy\_safety\_slice} & \texttt{policy\_bridge} & \texttt{policy\_safe\_response} & 我现在有点想直接看完整题解和代码，不然不知道从哪里继续。 我不是想跳过学习，只是《[CSP-J 2024] 接龙》卡... & \texttt{safe\_refusal} & recent\_dialogue\_ends\_with\_assistant; has\_problem\_context \\
\bottomrule
\end{xltabular}
\endgroup

\section{Appendix B. Real-Student AIChat External-Validation Boundary}
...
The field \texttt{observed\_current\_aichat\_response} means the current system response already shown by online AIChat. In the Chinese coach-review template, the same field is labeled \texttt{线上已展示 AIChat 回复（观察项，非实验条件）}. This field must not be described as a baseline, experimental condition, control arm, online comparison arm, or Repair output. It is an observed response used to test whether the benchmark rubric can be applied to real dialogue-state cases.
...
\bibliography{eait_v0_13_references_20260520}
\end{document}
