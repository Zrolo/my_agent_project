# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.2
...
Change from v0.1: this draft adds an anti-hallucination and unsupported-specificity rule after the first 20-case dev run showed that the tutor could invent concrete examples, variable meanings, coordinates, state dimensions, or data structures when the public freeze packet only contained title-level problem context. v0.2 is still a dev prompt, not a frozen prompt.
