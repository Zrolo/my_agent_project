# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.9
...
Change from v0.8: this draft replaces the hard forbidden-term mechanism with an abstract unsupported-specificity rule. The listed terms are retained only as examples of risky unsupported specificity, not as the main control mechanism. v0.9 is still a dev prompt, not a frozen prompt.
...
```json
{
  ...
  "unsupported_specificity_examples": [
    ...
    "例如：模型不能因为题名猜出特定算法、特定数据结构、特定状态形状、特定操作语义或特定证明关系"
  ]
}
```
...
16. In `title_only` context, the student-facing response should be at most two short sentences. It may add a brief opener such as "先别急着改代码", but the main action should stay close to one template.
...
18. In `title_only` context, generic phrases such as "一次操作", "一个变量", "已有样例", and "一步变化" are acceptable because they do not add unsupported problem content.
...
13. The repaired response should be no longer than two short sentences and should keep only one learner-owned next action.
