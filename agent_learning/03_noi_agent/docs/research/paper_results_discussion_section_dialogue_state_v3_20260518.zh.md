...
我们对 60 条高优先级分歧进行 priority adjudication，其中 `use_A=29`、`use_B=9`、`new_label=22`。这表明任一单独教练评分都不应被视为 gold，也说明 pedagogical judgment 在 student-ready、safe-ready 和 rank preference 上具有 rater sensitivity。因此后续结果同时报告 priority adjudication、slice analysis、paired uncertainty 和 rater-view sensitivity。
