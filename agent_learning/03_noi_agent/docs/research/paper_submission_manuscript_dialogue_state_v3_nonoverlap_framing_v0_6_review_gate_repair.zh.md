...
LLM tutors for competitive programming can give timely help, but useful help must preserve the learner's opportunity to complete the next reasoning step. Critical bridge leakage names a tutoring-specific failure in which a response prematurely supplies an intermediate bridge, such as a DP-state meaning, a binary-search predicate, or an update rule, even without final code or a complete solution. CP-MissingBridgeBench evaluates this problem through case-specific human review rather than final-answer disclosure alone. We evaluate 350 tutor responses from 7 anonymized offline harnesses across 50 reviewed dialogue-state cases. The primary scaffold analysis focuses on the 31-case `main_scaffold_eval` slice, corresponding to 217 tutor responses, while the remaining cases are reported as sensitivity analyses. Each case pairs problem context and recent dialogue with a rubric specifying the missing bridge, forbidden content, acceptable reveal, and expected next student action. Under the primary human-review view, DBox-inspired decomposition is a strong offline single-turn baseline; no-direct-solution prompting does not remove critical bridge leakage; and Bridge Contract compact + Guard/Repair shows favorable but bounded trends in overall quality and high-severity leakage control. In contrast to final-answer leakage work under adversarial attacks and DBox-style decomposition systems, this study evaluates turn-level tutoring and bridge preservation, not general tutor robustness or a decomposition product. Sensitivity, stress, and calibration analyses remain supporting evidence, and human review remains necessary. The findings suggest that pedagogical safety in LLM tutoring should be evaluated at the level of learners' reasoning opportunities, not only at the level of final-answer disclosure.
...
DBox-style work addresses a different educational design problem: how LLMs can support learners in decomposing algorithmic programming tasks. CP-MissingBridgeBench does not propose a DBox-style decomposition system and does not evaluate DBox as a deployed interactive system. It evaluates case-specific tutor responses, including a DBox-inspired offline single-turn condition used as a strong baseline, to ask whether each response preserves the learner's current missing reasoning bridge.
...
Prior work on rubric-based expert evaluation emphasizes that open-ended educational artifacts require explicit criteria if human judgment is to be interpretable. CP-MissingBridgeBench follows this logic by fixing, for each case, the success criteria, forbidden content, critical bridge boundary, acceptable reveal, and expected next student action before scoring. Coach A/B review and priority60 adjudication are treated as expert reference views with sensitivity reporting, not as a single definitive label. 【citation placeholders: rubric-framework-validation; AI-generated-content-expert-review; human-AI-assessment-support; tutor-response-human-annotation】
...
Prior work on algorithmic-programming scaffolding is close to CP-MissingBridgeBench because it treats decomposition as part of learning to solve programming problems. DBox-style work studies LLM-supported co-decomposition for algorithmic programming. This is a relevant comparison point for CP tutoring because decomposition can make hidden reasoning steps explicit and can structure the learner's next move. 【citation placeholders: DBox; algorithmic-programming-scaffolding; AI-tutor-evaluation-taxonomy; human-LLM-competitive-programming-benchmark】

The limitation is that decomposition support and leakage-boundary evaluation are different research objects. CP-MissingBridgeBench uses a DBox-inspired single-turn condition as a strong offline baseline, but it does not claim to reproduce the complete interactive DBox system or evaluate DBox as a deployed product. The paper's position is evaluative: it asks whether DBox-inspired and bridge-guided responses preserve critical bridges under case-specific human review.

### 2.6 LLM Graders And Calibration

Prior work on AI-supported assessment and LLM graders motivates automatic scoring as a possible way to scale feedback review, triage, or low-risk evaluation. At the same time, educational assessment work treats human judgment as especially important when the evaluated construct is nuanced, contextual, or high consequence. CP-MissingBridgeBench follows that distinction by treating DeepSeek-backed judging as auxiliary calibration rather than a replacement for human review. 【citation placeholders: AI-evaluator-human-raters; AI-assessment-support; automated-writing-evaluation; LLM-evaluator-reliability】
...
The case-level tutoring situation is the primary unit for slice assignment, while response-level outputs are generated by applying the 7 anonymized conditions to each case. Condition differences are therefore interpreted with same-case paired comparisons rather than as 217 independent cases.
...
The evidence hierarchy is therefore organized as follows:

| evidence layer | unit | count | claim role |
| --- | --- | ---: | --- |
| Dialogue-state corpus | case-level tutoring situations | 50 | reviewed evidence package; not a random sample of all CP tutoring |
| Offline harness outputs | response-level outputs | 350 | generated by applying 7 anonymized conditions to each case |
| Headline scaffold slice | case-level situations / response-level reviews | 31 / 217 | primary scaffold evaluation; paired same-case comparisons |
| Non-main slices and all-case summaries | case-level situations / response-level reviews | 19 / 133 | sensitivity and appendix evidence only |
| Real-student online pilot screening | candidate turns | 137 | ecological-validity screening pool; not deep annotation |
| Real-student online pilot candidates | selected candidate cases | 30 | pending consent/reporting gate; not a main result |
...
| `dbox_inspired_clean` | literature-inspired decomposition baseline | DBox-inspired offline single-turn scaffold; not the DBox interactive system. |
| `dbox_inspired_guard` | DBox-inspired + guard-instrumented variant | Guard-only 在当前主实验中主要提供 runtime leakage signal，不改写最终回复。 |
| `bridge_guided_dbox_style_guard` | bridge-guided decomposition variant | DBox-style format with bridge guidance only; not DBox reproduction; still a guard-instrumented condition. |
...
This study has six main limitations. First, the headline slice is 31 cases / 217 responses and should be interpreted as bounded expert-reviewed evidence, not exhaustive CP tutoring coverage; the 50 cases are curated high-risk dialogue-state cases rather than a random sample of all CP tutoring interactions. Second, student-ready, safe-ready, and rank outcomes are rater-sensitive, so sensitivity views are necessary. Third, priority60 adjudication is not a single definitive reference; it reduces uncertainty for high-priority disagreements but does not remove all rater differences. Fourth, DBox+Repair is a targeted 20-case sensitivity review, not a full 50-case double-coach condition. Fifth, DeepSeek-backed LLM-grader calibration has high critical false-negative risk and same-backend coupling, so automatic graders are unsuitable for high-stakes bridge-leakage adjudication. Sixth, the paper evaluates offline tutoring harnesses and does not validate online default AIChat or active-mode deployment; the real-student online pilot remains an ecological-validity screening design until consent/reporting gates are complete.
...
This manuscript uses the existing dialogue-state v3 evaluation set and its reproduction boundaries. The offline evidence package is organized as an expert-reviewed benchmark rather than as a student-intervention study. It does not modify online AIChat behavior, add active-mode deployment, change student-visible responses, or introduce new main-experiment conditions.

The real-student online pilot is treated as a separate ecological-validity design. It uses only our own AIChat / teaching-system data and excludes public community or third-party platform data. The current online pilot layer includes an aggregate log funnel and a lightweight candidate-turn screening pool; the 30 selected pilot candidate cases remain pending consent/reporting gate and should not be reported as deep-pilot evidence, student examples, or case-level findings until privacy and consent/status requirements are complete.

Data availability is constrained by tutoring privacy. Public-facing materials should include only anonymized aggregate statistics, schemas, analysis scripts, and paraphrased examples after consent/status approval. Complete student messages, complete code, complete AI responses, raw identifiers, hash mappings, hash salts, exact timestamps, school/account/phone/email identifiers, and individual student/problem hash tables should not be included in the public manuscript or open supplement. If additional materials are shared for review, they should be provided as restricted-access, privacy-reviewed artifacts rather than public raw logs.

AI writing assistance was used for drafting, editing, checklist generation, and evidence organization. The scientific claims, citation placeholders, numerical results, data boundaries, and final wording remain the responsibility of the human authors. Before submission, the manuscript still needs venue-specific checks for AI writing disclosure, privacy and data statements, citation verification, result-number verification, and human approval of any formal citations.
...
Audit note on `dialogue_v3_045_debugging_evidence`: this non-headline row remains in the original context-readiness audit slice and is not used for headline scaffold interpretation. A source-field discrepancy between the context-readiness audit and the reviewed candidate file is retained in the internal change log; no slice is reassigned and no main table is recomputed here.
