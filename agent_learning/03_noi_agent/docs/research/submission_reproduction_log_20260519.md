...
This document records the reproduction command results for dialogue-state v3 submission preparation. It adds no experiments, changes no online AIChat behavior, changes no prompt, and changes no main experiment data. Checkpoint interpretation:

- `dbbbd5c` = evidence content base and may appear in machine-readable manifest metadata.
- `33a5dd7` = evidence package gates checkpoint.
- `7baa54e` = integrity zh-pairs checkpoint, closing the current non-legacy bilingual pairing gap.
