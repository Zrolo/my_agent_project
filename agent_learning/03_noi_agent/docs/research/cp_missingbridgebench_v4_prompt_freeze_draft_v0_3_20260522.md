# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.3
...
Change from v0.2: this draft tightens the anti-hallucination rule after targeted dev rerun showed that the tutor could still offer candidate state shapes, invented example construction steps, or specific data-structure cues. v0.3 is still a dev prompt, not a frozen prompt.
...
13. When the packet says detailed context is restricted to private workbook/seed, treat all problem-specific facts beyond the title as unavailable. Use wording such as "用你当前代码里的变量" or "选择你已有的最小样例", not new invented facts.
