...
本文档在 `paper_submission_manuscript_dialogue_state_v3_eait_v0_4_storyline.zh.md` 基础上调整 framing，目标是避免与邻近的 final-answer leakage / adversarial student attack 工作以及 DBox-style algorithmic-programming decomposition 工作发生主张重叠。修改只涉及定位、叙事边界和术语澄清；不新增实验、不新增正式 citation、不修改任何数字、不改变 evidence class，也不修改线上 AIChat、active mode、prompt 或主实验数据。

## Abstract

LLM tutors for competitive programming can help students move through difficult reasoning steps, but the educational risk is not limited to final answers or complete solutions. Nearby tutor-leakage work has emphasized final-answer disclosure, often under adversarial student attacks, while DBox-style work studies LLM-supported algorithmic-programming decomposition. CP-MissingBridgeBench addresses a different gap: ordinary turn-level tutoring responses may prematurely complete a learner's case-specific intermediate reasoning bridge. We evaluate this critical-bridge leakage through a case-specific human-review benchmark. The dialogue-state v3 evidence package contains 50 reviewed cases, 7 anonymized offline tutoring conditions, and 350 AI responses. Each case pairs problem context and recent dialogue with a rubric defining the missing bridge, forbidden content, acceptable reveal, and expected next student action. Main claims are restricted to the 31-case `main_scaffold_eval` slice. Under the primary human-review view, DBox-inspired decomposition is a strong baseline; no-direct-solution prompting does not remove critical bridge leakage; and Bridge Contract compact + Guard/Repair shows favorable but bounded trends in overall quality and high-severity leakage control. These trends are not dominance claims. Sensitivity, stress, and calibration analyses remain supporting evidence, and human review remains necessary for high-risk bridge-leakage judgment.
...
Open-ended educational feedback is difficult to evaluate without a rubric. In CP-MissingBridgeBench, each case fixes success criteria, forbidden content, acceptable reveal, and expected student next action before scoring begins. Coach A/B review and priority60 adjudication are therefore treated as expert reference views with sensitivity reporting, not as a single definitive label. 【占位：rubric validation / expert review / human annotation】
...
What this supports: These findings support the use of human review with explicit sensitivity reporting. They show that expert judgment is informative but not reducible to one unproblematic label.

What it does not support: These results do not justify treating Coach A, Coach B, or priority60 adjudication as a single settled reference. They also do not remove the need to report uncertainty for student-ready, safe-ready, and ranking outcomes.
...
What it does not support: The table does not establish broad dominance of Bridge Contract over all baselines. It also does not validate online deployment, since all conditions are offline evaluation harnesses.
...
### 5.2 The Safety Framing Is Pedagogical And Tutoring-Specific

The safety framing in this paper is deliberately narrow. It concerns whether a tutor preserves a learner's reasoning opportunity in a competitive-programming turn. It is not a broad account of safety across all AI-supported educational settings, and it is not a benchmark for adversarial attacks. This scope matters because critical bridge leakage can occur in ordinary help-seeking dialogue, not only when a student tries to extract an answer.
