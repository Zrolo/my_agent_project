...
| Leakage Guard can detect critical bridge leakage and provide a runtime intervention signal | Leakage Judge and guard pipeline exist; the current guard-only condition does not rewrite student-visible final responses except for block fallback | Need to prove the guard is accurate and not relying on oracle forbidden content; final-leakage reduction claims require repair/block evidence or same-candidate before/after tests | Predicted-only guard; coach leakage labels; precision/recall/F1; false-positive rewrite rate; same-candidate repair stress | `needs_judge_validation` |
