...
本协议说明如何从 137 条 real-student online AIChat substantial candidate turns 中选择 30 条 deep pilot annotation cases。它不修改 dialogue-state v3 主实验，不新增主实验 condition，不新增 baseline，不重算主表，不改变 evidence class，不上线 active mode，不改变学生可见回复，也不把 137 或 30 写成 learning outcome study。

137 条 candidate turns 是 lightweight screening pool，不是深度标注样本。30 条 selected cases 是 deep annotation sample，不是全部线上 AIChat 数据。该 deep sample 只用于 taxonomy / rubric ecological validity check，可作为 Discussion / Appendix evidence，不作为 main result。
...
- 30 条 deep pilot case annotation candidates。
- 每条 case 使用 `real_student_online_case_schema_v1.json` 做完整 case-specific rubric annotation。
- 每条 selected deep case 在 screening form 中必须填写非空 `candidate_selection_reason`，说明它被选入 deep annotation sample 的覆盖性或可标注性理由。
...
优先让 30 条 deep sample 覆盖多个 rough bridge families，包括但不限于：
...
30 条 deep sample 不应只选上下文最完整的 case。建议覆盖：
...
deep sample 应包含真实线上 AIChat 中常见的高风险帮助场景：
...
选择 30 条 deep sample 时不得按以下因素选择：
...
6. 将 30 条转换为 `real_student_online_case_schema_v1.json` 的 deep annotation cases。
7. 完成 privacy review、consent/status 检查和 coach annotation。
8. 在 reporting template 中报告三层漏斗，并明确 137 是 screening pool、30 是 deep annotation sample。

## 报告边界句

```text
We use a three-layer real-student online AIChat pilot design. The online log corpus summary describes 1156 message rows, 87 sessions, and 578 paired user-assistant turns. A lightweight screening layer identifies 137 substantial candidate turns across 59 candidate sessions. From this screening pool, 30 cases covering 11 hashed students and 15 hashed problems are selected for privacy-reviewed deep annotation. The 137 candidate turns are not deeply annotated, and the 30 deep cases are not the full online corpus or a learning-outcome study.
```
