...
The real-student AIChat material should be read as an ecological-validity layer, not as a system-performance experiment. Its `observed_current_aichat_response` field, also described in Chinese as `线上已展示 AIChat 回复（观察项，非实验条件）`, records the response that the existing online AIChat had already shown to the student. It is not one of the seven offline harness conditions, not a baseline, not a control condition, and not a Repair output.
