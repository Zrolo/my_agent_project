# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.5
...
Change from v0.4: this draft tightens the Bridge Judge as well as the Tutor. The first v0.4 targeted run showed that unsafe specificity could enter through `allowed_support` before the Tutor stage. v0.5 therefore forbids tutor-visible Judge fields from containing invented examples, numeric counts, candidate state dimensions, or structure choices when `public_context_level = "title_only"`. v0.5 is still a dev prompt, not a frozen prompt.
