...
本文档提供 dialogue-state v3 manuscript 可直接复制的 table source。它不新增实验、不修改数据、不接入线上 active mode。表格复制或压缩自已有 paper-ready result reports，必须保留下列解释边界。

全局表格边界：

- 主 headline tables 只使用 `main_scaffold_eval`。
- 主参考口径是 `priority60 adjudicated + Coach A`，不是 final gold。
- all-50 averages 只作为 appendix / sensitivity。
- Guard-only 是 instrumentation / runtime signal，不是 final-response rewrite。
- Repair 因果证据来自 same-candidate stress testing。
- DBox+Repair 是 targeted fairness sensitivity，不是 full main condition。
- DeepSeek LLM grader calibration 只能作为 auxiliary。
...
```text
Dialogue-state v3 separates cases by paper use. The main headline comes only from the 31-case main_scaffold_eval slice; other slices and all-50 aggregates are sensitivity or appendix views.
```
...
| `dbox_inspired_guard` | DBox-inspired + guard-instrumented variant | Guard-only 提供 runtime leakage signal；不改写 final responses。 |
| `bridge_guided_dbox_style_guard` | Bridge-guided decomposition variant | 使用 bridge signal 引导 DBox-style response；仍是 guard-instrumented。 |
...
| `bridge_contract_compact_guard_repair` | Bridge Contract compact + Guard/Repair method | Student-visible response 可能来自 Repair；主均值只说明 condition-level trends。 |
...
- `paper_methods_evaluation_dialogue_state_v3_20260518.zh.md`
- `baseline_protocol_v1.zh.md`
- `paper_results_interpretation_guardrails_20260517.zh.md`
...
不要把 appendix-only all-50 sensitivity 移入 main headline。
