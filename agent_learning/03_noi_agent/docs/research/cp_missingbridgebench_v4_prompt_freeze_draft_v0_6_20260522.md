# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.6
...
Change from v0.5: this draft adds a title-only scaffold template mode. When the public packet contains only title-level context, the Tutor should choose from a small set of learner-owned action templates instead of composing free-form problem-specific examples. v0.6 is still a dev prompt, not a frozen prompt.
...
```json
{
  ...
  "title_only_scaffold_templates": [
    "用一句话写出你当前状态/变量/维护对象的含义",
    "选择你已有的最小样例，记录你当前代码中的一个变量每一步的值",
    "写出你认为每一步后应该保持的一条性质",
    "说明你当前实现中一次操作前后哪些量应该改变"
  ]
}
```
