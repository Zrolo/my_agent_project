# CP-MissingBridgeBench v4.1 Prompt Freeze Draft v0.4
...
Change from v0.3: this draft adds an explicit `public_context_level` signal. When `public_context_level = "title_only"`, the tutor must not add illustrative problem-specific examples, numbers, coordinates, object counts, binary strings, state dimensions, or structure names that are not present in the packet. v0.4 is still a dev prompt, not a frozen prompt.
