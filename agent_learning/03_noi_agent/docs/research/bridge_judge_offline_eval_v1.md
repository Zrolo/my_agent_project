...
Do not switch Kimi judge calls back to `max_tokens`. If Kimi returns truncated content, raise `NOI_BRIDGE_JUDGE_MAX_TOKENS` / `NOI_LEAKAGE_JUDGE_MAX_TOKENS` / `NOI_REPAIR_RESPONSE_MAX_TOKENS`; if it returns `empty_content`, treat it as provider-output instability and inspect the raw stage error before changing schema logic.
