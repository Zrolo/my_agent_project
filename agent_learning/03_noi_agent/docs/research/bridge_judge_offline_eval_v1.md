...
The default offline judge output budget is `4096` generated tokens for Bridge Judge, Leakage Judge, and Repair Response. This is deliberately larger than the previous 1024-token smoke setting because schema JSON plus natural-language evidence can otherwise be truncated. Override with `NOI_BRIDGE_JUDGE_MAX_TOKENS`, `NOI_LEAKAGE_JUDGE_MAX_TOKENS`, or `NOI_REPAIR_RESPONSE_MAX_TOKENS` when running cost-sensitive smoke tests.
