# CP-MissingBridgeBench: EAIT-Facing Manuscript Draft v0.1

## 使用边界

本文档将 `paper_submission_manuscript_dialogue_state_v3_20260519.zh.md` 重排并改写为 Education and Information Technologies 面向的中文论文初稿 v0.1。它不新增实验、不新增 citation、不修改任何数字、不改变 evidence class，也不修改线上 AIChat、active mode、prompt 或主实验数据。本文是 evaluation framework / benchmark paper 草稿，不是线上部署系统论文。

## Abstract

Large language models are increasingly used as programming tutors, yet pedagogically useful help is not captured by direct-answer avoidance alone. In competitive programming, students often need support before completing a local reasoning bridge, but premature completion of that bridge can undermine learning even when no final code or full solution is provided. We introduce CP-MissingBridgeBench, a turn-level evaluation framework for missing-bridge scaffolding and critical-bridge leakage in LLM tutoring. Each case combines problem context, recent dialogue, a student message, and a case-specific rubric specifying success criteria, forbidden content, acceptable reveal, and expected next student action. The dialogue-state v3 evidence package contains 50 reviewed candidate cases, 7 anonymized offline tutoring harnesses, and 350 AI responses evaluated through expert blind review, priority adjudication, paired uncertainty, slice analysis, same-candidate Repair stress testing, targeted DBox+Repair fairness sensitivity, and DeepSeek-backed LLM-grader calibration. The main headline is restricted to the 31-case `main_scaffold_eval` slice. Under the primary human-review view, CP-MissingBridgeBench reveals quality-safety-burden trade-offs: DBox-inspired decomposition is a strong baseline, no-direct-solution prompting does not eliminate critical bridge leakage, and Bridge Contract compact + Guard/Repair shows favorable but bounded overall-quality and high-severity leakage-control trends. Sensitivity, stress, and calibration results are reported as supporting evidence classes, not main-result substitutes. Human review remains necessary.

## 1. Introduction

Students need help before completing a reasoning bridge, but premature completion undermines learning. 这一直是 AI 辅导中最难处理的张力之一：学生确实需要提示、澄清和下一步脚手架，但如果 tutor 直接替学生跨过当前最关键的中间推理，学习活动就可能从主动建构变成被动接收。

在算法竞赛辅导中，这个张力尤其明显。学生的问题往往很短，近期对话和局部代码中却包含丰富状态：他可能已经理解了题意，但还没有理解状态含义；可能已经想到二分，却还没有说清 check 谓词；也可能在调试局部代码时缺少能定位错误的证据。一个回复即使没有给最终代码，也可能通过过完整的提示、微型例子或反问，提前说穿学生本应自己推出的关键桥梁。

本文将学生当前理解与下一步有效解题动作之间尚未跨过的局部推理称为 missing bridge。critical bridge leakage 则指 tutor 没有直接给完整题解或代码，却提前补完了当前 missing bridge。它不同于普通 answer leakage：一个回复可以不泄露最终答案，但仍然泄露最关键的中间推理；也可以提供背景、轻提示或聚焦问题，而不越过学生应当完成的桥梁。

因此，评估 LLM tutor 不能只问“有没有直接给答案”。对 turn-level tutoring 来说，更关键的问题是：回复是否贴合学生当前状态，是否足够帮助学生迈出下一步，是否避免过早完成当前 missing bridge，以及是否把下一轮学生行动控制在合理负担内。
...
LLM feedback and scaffolding 相关工作说明，生成式模型可以生成大量、及时、形式多样的反馈，但 AI feedback 与教师反馈并不等价。本文沿用这种谨慎立场：LLM tutor 的回复不能只按流畅度、礼貌程度或信息量评价，而要看它是否保留学生应完成的认知工作。no-direct-answer 或 no-direct-code 规则是必要约束，但不足以判断脚手架是否教学上安全。
...
Algorithmic-programming scaffolding and benchmarks 为本文提供了更接近的领域参照。DBox-inspired decomposition 被用作 strong baseline，但本文不声称复现 DBox 的交互式、多轮、step-tree 系统；MRBench 类 pedagogical benchmark 则说明 tutor evaluation 可以围绕教育维度与人类标注组织。CP-MissingBridgeBench 的定位是 CP-specific bridge / leakage benchmark，而不是通用 tutoring taxonomy 或完整线上 tutor 系统。
...
主 headline 只使用 `main_scaffold_eval`。`main_eval_with_caution`、`clarification_safety_slice`、`policy_safety_slice` 和 all-50 aggregate 只作为 sensitivity / appendix evidence。
...
这些 condition 都是 offline evaluation harness，不等于线上默认 AIChat，也不表示线上 active mode 已接入这些模块。
