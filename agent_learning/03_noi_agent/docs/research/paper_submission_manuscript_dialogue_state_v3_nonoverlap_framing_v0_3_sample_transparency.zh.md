...
本文档在 `paper_submission_manuscript_dialogue_state_v3_nonoverlap_framing_v0_3.zh.md` 基础上增强样本规模与 slice 透明性。修改只涉及 case-level / response-level 样本说明、slice 表、Results 入口、Limitations 和 Appendix case inventory；不新增实验、不新增正式 citation、不修改任何已有数字、不改变 evidence class，也不修改线上 AIChat、active mode、prompt 或主实验数据。
...
| `dialogue_v3_045_debugging_evidence` | `policy_safety_slice` | `debugging_bridge` | `debugging_evidence` | 我知道要调试，但不知道“最小反例”该怎么选。能不能先让我挑一个很小输入，看哪个中间量应该是多少？我现在不是想改完整代... | `safe_refusal` | recent_dialogue_ends_with_assistant; has_code_excerpt; has_problem_context |
