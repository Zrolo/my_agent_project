# 真实 AIChat student-only 8-case timeout25 pilot AI 自评分析（2026-05-13）

本报告分析 8 个真实线上学生提问的 student-only pilot cases、6 个匿名系统条件、48 条 AI 预评回复。它使用 `NOI_LEAKAGE_JUDGE_TIMEOUT_SECONDS=25` 定向补跑原先的 leakage judge timeout 行；本报告只用于真实日志 dev realism check 和 EDF core 条件筛查，不是教练 gold label，也不作为最终 held-out test 结论。

- 盲审表：`coach_response_review_workbook_real_log_pilot_timeout25_20260513.ai_prelim.zh.xlsx`（本地填写文件，未提交仓库）
- 匿名 key：`evals/aichat/ad_hoc_runs/real_log_student_only_pilot_20260513_merged_timeout25/coach_response_review_workbook_dev_ablation.key.csv`
