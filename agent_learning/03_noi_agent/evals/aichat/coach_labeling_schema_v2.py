...
FIELD_HELP = {
    ...
    "problem_statement": "教练判断 missing bridge 前应先看必要题面；正式公开材料不要转载完整题面。",
    "problem_source_url": "真实题源链接，用于教练回看完整题面和样例。",
    "student_scaffold_followability": "仅用于 dialogue-state v3。根据学生对上一轮 AI 脚手架的回复判断：F1 能跟上，F2 部分跟上，F3 跟得吃力，F4 基础断层。",
    "followability_evidence_quote": "仅用于 dialogue-state v3。必须引用学生对上一轮 AI 提问的原话片段，作为 F1-F4 标签证据。",
    "expected_tutor_move": "仅用于 dialogue-state v3。表示当前回复更适合推进、澄清、拆小步、补前置概念或安全重定向。",
}
...
BRIDGE_SUBTYPES = {
    ...
    "method.monotone_answer_search_signal": "单调候选答案搜索信号：识别候选答案和可行性判断之间的单调关系",
    "method.stateful_subproblem_signal": "状态化子问题信号：判断哪些历史信息、资源限制或子问题边界需要被保留",
    "method.local_choice_rule_signal": "局部选择规则信号：识别可排序、可交换、可支配或局部选择的线索",
    "state.table_or_memo_cell_semantics": "表格/记忆化格子语义：定义表格、数组或记忆化单元的下标和值含义",
    "state.encoded_set_semantics": "编码集合状态语义：定义二进制位、压缩编码或集合字段表示的选择/资源/状态",
    "state.deferred_update_semantics": "延迟更新语义：说明某个标记、缓存或待处理字段保存了什么尚未传播的信息",
    "state.augmented_process_state": "扩展过程状态：除当前位置/节点外还要携带资源、权限、阶段或历史信息",
    "state.failure_link_or_prefix_semantics": "回退/前缀状态语义：失配、回退或共享前缀信息在辅助数组/状态中表示什么",
    ...
    "predicate.feasibility_truth_direction": "可行性真假方向：候选、状态或局部判断对应的 true/false 语义",
    "predicate.boundary_update_direction": "边界推进方向：根据判定结果收缩、推进或保留哪一侧候选",
    "predicate.improvement_update_condition": "改进式更新条件：什么时候一个候选、边、转移或局部结果可以改进当前值",
    "predicate.obsolete_candidate_guard": "过期候选判断：多候选结构中弹出的候选是否仍与当前最优/当前状态一致",
    "predicate.local_if_boundary_condition": "局部 if/边界条件：某个局部条件该判断什么才合法",
    "ordering.single_use_update_order": "单次使用更新顺序：避免同一轮刚更新的状态被再次读取造成重复使用",
    "ordering.rolling_array_overwrite_order": "滚动数组覆盖顺序：压维后如何避免覆盖仍需使用的旧状态",
    "ordering.subproblem_size_dependency_order": "子问题规模依赖顺序：先处理更小、更短或前置子问题以满足依赖",
    "ordering.topological_dependency": "拓扑依赖顺序：先处理前驱，再处理后继或当前状态",
    "aggregation.path_contribution_marking": "路径/区间贡献标记：多次贡献如何压缩到少量边界或节点再汇总",
    "aggregation.cumulative_range_query": "累计量区间查询：区间贡献如何由累计值相减或组合得到",
    "aggregation.inclusion_exclusion_query": "容斥式区域查询：多维、矩形或重叠区域如何用加减还原",
    "aggregation.boundary_delta_update": "边界差量更新：区间/范围影响如何只在边界记录再统一还原",
    "ds.priority_candidate_operation_mapping": "优先级候选操作映射：题目中的当前最优/最小/最大候选如何对应取出和插入",
    "ds.component_merge_query_mapping": "连通分量合并/查询映射：关系合并、代表元和连通性查询如何对应结构操作",
    "ds.dominated_candidate_stack_mapping": "被支配候选栈映射：比较关系如何决定保留、弹出和记录答案",
    ...
    "implementation.traversal_back_edge_guard": "遍历回走保护：树/图遍历中如何避免沿已处理关系回走或重复访问",
    ...
    "correctness.local_choice_exchange_argument": "局部选择交换论证：说明某个局部选择、排序或替换为什么不亏",
    "correctness.settled_candidate_invariant": "已确定候选不变量：说明为什么某个当前候选一旦被选定就不会再被更优结果推翻",
...
BRIDGE_SPECIFIC_FORBIDDEN_CONTENT = {
    ...
    "no_exact_check_condition": "不能直接给完整判定条件：避免说穿候选、状态或局部条件如何判真/判假",
    "no_exact_boundary_update_rule": "不能直接给完整边界更新规则：避免说穿下一步收缩、推进或保留哪一侧候选",
    "no_exact_guard_condition": "不能直接给完整守卫/跳过条件：避免替学生补完过期、非法或重复候选判断",
    "no_exact_modeling_plan": "不能直接给完整建模方案：避免一次性给点、边、状态、约束",
    "no_exact_greedy_rule": "不能直接给完整贪心准则和证明：避免直接暴露选择规则",
    "no_full_invariant_proof": "不能直接给完整不变量/正确性证明：避免替学生补完为什么一定成立的核心理由",
    "no_exact_contribution_formula": "不能直接给完整贡献公式：避免说穿前缀、差分、树上标记等公式",
    "no_exact_iteration_template": "不能直接给完整循环/遍历模板：避免补完整顺序和边界",
    "no_exact_data_structure_template": "不能直接给完整数据结构操作模板：避免完整套出当前结构的查询、更新、维护序列",
    "no_complete_local_condition": "不能直接补完整局部条件：避免替学生补完关键 if 或边界",
    "no_fully_worked_micro_trace": "不能给完整推演轨迹：避免把微型例子从引导观察变成直接演示关键桥",
}
