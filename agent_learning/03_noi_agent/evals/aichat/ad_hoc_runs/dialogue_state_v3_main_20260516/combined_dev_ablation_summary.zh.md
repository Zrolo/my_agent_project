...
| 自动进入主结果候选 | False |
| Dev Gate 需要复查 | True |
| Dev Gate 原因 | incomplete_or_error_rows, critical_bridge_leakage, repair_still_leaks, final_static_answer_slot_risk, final_static_filled_trace_risk, final_static_worked_example_risk |
| 学生状态准确率 | n/a |
| 桥梁大类准确率 | n/a |
| 知识焦点准确率 | n/a |
| 已注册知识焦点准确率 | n/a |
...
| 焦点越界率 | n/a |
| 自相矛盾率 | n/a |
| 平均 Prompt Token 估计 | 101.903 |
| 平均 LLM 调用次数 | 2.294 |
| 候选回复静态风险率 | 0.362 |
| 候选回复答案槽位静态风险率 | 0.091 |
| 候选回复已填 trace 静态风险率 | 0.197 |
| 候选回复完整微例静态风险率 | 0.106 |
| 最终回复静态风险率 | 0.359 |
| 最终回复答案槽位静态风险率 | 0.091 |
| 最终回复已填 trace 静态风险率 | 0.194 |
| 最终回复完整微例静态风险率 | 0.106 |
...
| 平均 LLM 调用次数 | 3.083 |
| 自动进入主结果候选 | False |
| Dev Gate 原因 | incomplete_or_error_rows, critical_bridge_leakage, final_static_answer_slot_risk, final_static_filled_trace_risk, final_static_worked_example_risk |
| 最终回复静态风险率 | 0.354 |
| 最终回复答案槽位静态风险率 | 0.125 |
| 最终回复已填 trace 静态风险率 | 0.188 |
| 最终回复完整微例静态风险率 | 0.083 |
...
| 最终回复静态风险率 | 0.298 |
| 最终回复答案槽位静态风险率 | 0.085 |
| 最终回复已填 trace 静态风险率 | 0.170 |
| 最终回复完整微例静态风险率 | 0.085 |
| 总延迟 P50 ms | 24263.981 |
