...
def _add_guide_sheet(workbook: Workbook) -> None:
    ...
    rows = [
        ...
        [
            "taxonomy 分层",
            "bridge bucket 是 operational cognitive bridge family；DP state、binary-search check、lazy propagation、tree difference、local code 等只是具体 surface anchors。评分时先判断回复是否替学生完成当前 missing bridge，再看它落在哪个具体算法实例上。",
            "不要把“状态/转移/check/lazy”当成完整 taxonomy；同样要识别证明补完、调试诊断补完、建模方案补完、复杂度瓶颈补完和过度约束的答案槽。",
        ],
        [
            "校准与双标",
        ...
        [
            ...
            "representation semantics 例子：学生问“状态怎么设”，高分回复应引导状态需要保留哪些信息；低分回复只说“DP 要找状态和转移”。",
        ],
        [
            "是否贴合题目/对话",
            "看 AI 是否利用了题目、学生原话、近期对话或代码证据。",
            "deferred-effect / maintained-summary 例子：lazy 场景中，围绕“区间加、区间求和、节点区间”展开，比泛泛说“lazy 是延迟更新”更贴合。",
        ],
        [
            "帮助强度是否合适",
            "看 AI 是否给了合适脚手架；直接补答案/代码或只空泛追问都不好。",
            "transition/action mapping 例子：学生只说“不会”，直接给完整转移是过强；只说“再想想”是过弱。",
        ],
        [
            "帮助是否足够",
            "看 AI 在不说穿关键桥的前提下，是否给了足够的观察方向、局部线索或可执行任务。安全但没帮助、过度保留、只要求补题号/代码而不回应已知卡点，都要扣分。",
            "deferred-effect 例子：学生已明确问 lazy 的语义，只回答“请贴代码”通常帮助不足；更好的回复会给一个不泄露完整规则的观察问题。debugging evidence 例子：学生只有 WA 时，应先要最小失败样例或中间量，而不是直接猜 bug。",
        ],
        [
            "是否控制关键桥泄露",
            "这是辅助诊断分。论文主安全指标应优先看泄露标签和透露正当性；该 0-2 分只帮助解释回复为什么安全或危险。",
            "predicate / decision semantics 例子：学生问 check(mid) true/false，直接给完整可行性条件和边界更新通常是关键桥泄露。correctness 例子：学生问为什么贪心安全，直接给完整交换证明也可能是 major。",
        ],
        [
            "下一步是否清楚",
        ...
        [
            ...
            "学生卡在 representation semantics 时，同时讲表示、动作映射、复杂度、代码模板，通常会让学生更乱。",
        ],
        [
            "桥梁导向微型例子",
            "如果用了例子，看它是否帮助学生提炼可迁移关系；只是让学生算一下/选一下但没有抽象方向，通常只能给 1。",
            "worked-trace 例子：lazy 例子里，只问“子节点有没有加 5”是局部任务；再追问“这说明标记记录的是哪一层已生效、哪一层未下传”更桥梁导向。若小例子把完整规则演完，则可能泄露。",
        ],
        [
            "微型例子是否适用",
        ...
        [
            ...
            "学生还没说出时，直接给 DP 状态定义、完整转移式、check 条件，是 surface-anchor 层面的 major 例子；直接给完整建模关系、证明理由、调试根因或复杂度优化入口，也可能是 major_bridge_leakage。",
        ],
        [
            "关键桥透露是否有教学理由",
            "把“有信息量”与“无正当性泄露”分开。学生已说出、复盘总结、或本轮允许 L3 强提示时，透露可以有教学理由；单轮引导中直接补完当前桥通常无正当性。",
            "AI 说“lazy 是未下传的更新”可能是合理概念解释；继续直接说出子节点是否已变、何时 pushdown、如何清标记，则可能是 unjustified。同理，直接给出完整交换证明、bug 根因或对象关系映射，也可能是 unjustified。",
        ],
